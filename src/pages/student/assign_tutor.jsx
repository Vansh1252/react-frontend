...
import '../../styles/assign_tutor.css';
...
function AssignTutor() {
  ...
  // Fetch list of tutors on component mount
  useEffect(() => {
  ...
  // Fetch available slots from backend when tutorId or durationMinutes changes
  ...
  const calculateProgress = () => {
    let totalFields = 4; // tutor, timezone, duration, days
    let filledFields = 0;

    if (tutorData.tutorId) filledFields++;
    if (tutorData.timezone) filledFields++;
    if (tutorData.durationMinutes) filledFields++;
    if (Object.keys(tutorData.selectedSlots).some(day => tutorData.selectedSlots[day].length > 0)) filledFields++;

    return Math.round((filledFields / totalFields) * 100);
  };

  const handleSkip = async () => {
    toast.success("Student created successfully (without tutor).");
    navigate('/student');
  };
